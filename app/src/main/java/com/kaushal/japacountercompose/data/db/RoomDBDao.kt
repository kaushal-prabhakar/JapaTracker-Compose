...
interface RoomDBDao {
    ...
    suspend fun getMyJapas(): List<JapaInfoEntities>

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun addNewJapa(japaInfoEntities: JapaInfoEntities): Long

    @Query(
        "UPDATE Japa_Info SET CurrentCount = :newCount, LastUpdatedValue = :updatedValue, " +
                "LastUpdatedType = :updatedType WHERE JapaName = :name"
    )
    ...
    suspend fun getCurrentCount(name: String)

    @Query("SELECT * FROM Japa_Info WHERE JapaName = :name")
    suspend fun getJapaDetails(name: String)

    @Query(
        "UPDATE Japa_Info SET CurrentCount = :newCount, LastUpdatedValue = :updatedValue, LastUpdatedType = :updatedType, " +
                "LastUpdatedTime = :time WHERE JapaName = :name"
    )
    ...
    @Query(
        "UPDATE Japa_Info SET CurrentCount = 0, LastUpdatedValue = 0, LastUpdatedType = :updatedType " +
                "WHERE JapaName = :name"
    )
    suspend fun resetJapaCounter(name: String, time: LocalDateTime, updatedType: UpdateType)
